...
/// Package booking summary view for appointment confirmation
/// Displays all booking details and handles final package booking confirmation
@MainActor
struct PackageBookingSummaryView: View {
    
    // MARK: - Properties
    let packageDetails: HealthPackage
    ...
    private let labInfo = LabFacility(
        ...
        services: ["Blood Work", "Comprehensive Health Checkup"],
        reviewCount: 245,
    ...
    private var packageDetailsCard: some View {
        VStack(alignment: .leading, spacing: HealthSpacing.md) {
            HStack(spacing: HealthSpacing.sm) {
                // Package icon
                Text(packageDetails.icon)
                    .font(.system(size: 24))
    ...
    private var testsSummaryCard: some View {
        VStack(alignment: .leading, spacing: HealthSpacing.md) {
            HStack(spacing: HealthSpacing.sm) {
                ...
                Text("\(packageDetails.totalTests) tests")
            ...
            LazyVGrid(
                ...
                ForEach(packageDetails.testCategories.prefix(4), id: \.id) { category in
            ...
            if packageDetails.testCategories.count > 4 {
                Text("+\(packageDetails.testCategories.count - 4) more categories")
        ...
        VStack(alignment: .leading, spacing: HealthSpacing.md) {
            ...
            HStack {
                ...
                VStack(alignment: .trailing, spacing: 2) {
                    Text(packageDetails.formattedPrice)
                        .healthTextStyle(.title3, color: HealthColors.primary)
                        .fontWeight(.bold)
                    
                    if let originalPrice = packageDetails.originalPrice {
            ...
            if packageDetails.savings > 0 {
                HStack {
                    ...
                    Text(packageDetails.formattedSavings)
                        .healthTextStyle(.subheadline, color: .green)
                        .fontWeight(.semibold)
                }
                .padding(.vertical, HealthSpacing.xs)
                .padding(.horizontal, HealthSpacing.sm)
...
// MARK: - Package Booking Success View

@MainActor
struct PackageBookingSuccessView: View {
    let packageDetails: HealthPackage
    let selectedDate: Date
    let selectedTimeSlot: String
            ...
            VStack(spacing: HealthSpacing.xxxl) {
                ...
                VStack(spacing: HealthSpacing.xl) {
                    ...
                    VStack(spacing: HealthSpacing.md) {
                        Text("Package Booking Confirmed!")
                            .healthTextStyle(.title1, color: HealthColors.primaryText)
                            .multilineTextAlignment(.center)
                        
                        Text("Your health package appointment has been successfully booked. We'll send you preparation instructions and a confirmation shortly.")
                            .healthTextStyle(.body, color: HealthColors.secondaryText)
                            .multilineTextAlignment(.center)
                    }
                }
...
// MARK: - Previews
struct PackageBookingSummaryView_Previews: PreviewProvider {
    static var previews: some View {
        NavigationStack {
            PackageBookingSummaryView(
                packageDetails: HealthPackage.sampleComprehensive(),
                selectedDate: Calendar.current.date(byAdding: .day, value: 1, to: Date()) ?? Date(),
                selectedTimeSlot: "9:30 AM - 11:30 AM"
            )
        }
