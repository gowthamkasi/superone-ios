...
final class ProfileViewModel {
    ...
    func updateProfile(
        ...
        Task {
            ...
            do {
                ...
                await MainActor.run {
                    print("🔍 ProfileViewModel: Processing API response")
                    print("   response.success: \(response.success)")
                    print("   response.data exists: \(response.data != nil)")
                    print("   response.data?.user exists: \(response.data?.user != nil)")
                    print("   userProfile exists: \(userProfile != nil)")
                    
                    if let updatedUserProfile = response.data?.user,
                       let currentUser = userProfile {
                        print("✅ Both updatedUserProfile and currentUser are available")
                        
                        // Direct assignment since backend sends undefined/null when no value exists
                        let optionalHeight = updatedUserProfile.height
                        let optionalWeight = updatedUserProfile.weight
                        
                        print("🔍 Profile update data extracted:")
                        print("   firstName: '\(updatedUserProfile.firstName)'")
                        print("   lastName: '\(updatedUserProfile.lastName)'")
                        print("   email: '\(updatedUserProfile.email)'")
                        print("   mobileNumber: '\(updatedUserProfile.mobileNumber ?? "nil")'")
                        print("   height: \(optionalHeight?.description ?? "nil")")
                        print("   weight: \(optionalWeight?.description ?? "nil")")
                        ...
                        print("✅ ProfileViewModel: User profile successfully updated")
                        print("   New profile name: '\(userProfile?.name ?? "nil")'")
                        ...
                        print("✅ ProfileViewModel: Success feedback triggered")
                        ...
                        print("❌ ProfileViewModel: Failed to extract user data from response")
                        print("   response.data?.user == nil: \(response.data?.user == nil)")
                        print("   userProfile == nil: \(userProfile == nil)")
                        
                        // This would be the source of the error
                        isUpdatingProfile = false
                        updateProfileErrorMessage = "Unable to process server response. Please try again."
                        showUpdateProfileError = true
    ...
    func updateProfile(_ profile: User) {
        ...
        // Direct comparison since backend handles undefined properly
